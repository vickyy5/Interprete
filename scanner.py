...
from tokeen import Token    # importar la clase Token del módulo tokeen
...
class Scanner: #Clase Scanner
    ...
    def ScanTokens(self): # -> list[TipoToken]:
        ...
        for line in self.source:
            ...
            line1 +=" "
            print(f"{line1}")
            for char in line1: # Ciclo for que recorre cada caracter en la línea ya limpia
                #print(f"{estado} -> {char}")
                match self.estado: # Inicia un bloque de comparaciones de acuerdo al valor de la variable estado
                    case 0:
                        if char == "<":  # Dependiento del caracter actual el en este caso es "<", se cambia el valor de estado a 1
                            ...
                            self.estado = 8
                        else: # Si el carácter actual no es ninguno de los anteriores:
                            pass    # No hacemos nada y pasamos al siguiente carácter.
        ...
        self.tokens.append(Token(TipoToken.EOF,None,None,self.__linea)) #Se termina el archivo y se agrega el token EOF
            ...
            match self.estado: #Define un bloque 'match' que depende del estado actual
                case 0:
                    try:
                        ...
                        control = False # Si se llega al final de 'cadena', se acaba el bucle
                # Estado 1: se ha encontrado un símbolo
                case 1:
                    try:
                ...
                case 2:
                    try:
                        ...
                        else:  # Si el siguiente caracter no es un dígito ni un punto
                            #print(cadena[char+1])
                            clean_str += f" {current}" # Se agrega la cadena current a la cadena limpia, precedida por un espacio
                            current = "" # Se vacía la cadena current
                            char += 1 # Se aumenta el índice del caracter actual en 1
                ...
                case 3:
                    try:
                        ...
                        control = False  # Se agrega la cadena current a la cadena limpia, preced
